...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		// 階層構造
		Matrix4x4 worldMatrixA = MakeAffineMatrix(scales[0], rotates[0], translates[0]);
		Matrix4x4 worldMatrixB = MultiplyXYZ(MakeAffineMatrix(scales[1], rotates[1], translates[1]), worldMatrixA);
		Matrix4x4 worldMatrixC = MultiplyXYZ(MakeAffineMatrix(scales[2], rotates[2], translates[2]), worldMatrixB);
		
		
		Sphere sphere0{
			{worldMatrixA.m[3][0],worldMatrixA.m[3][1],worldMatrixA.m[3][2]},
		    0.05f,
		};
		Sphere sphere1{
			{worldMatrixB.m[3][0],worldMatrixB.m[3][1],worldMatrixB.m[3][2]},
			0.05f,
		};
		Sphere sphere2{
			{worldMatrixC.m[3][0],worldMatrixC.m[3][1],worldMatrixC.m[3][2]},
			0.05f,
		};

		// NDC(正規化デバイス座標系)
		Vector3 ndcVertexA = Transform(sphere0.center, worldViewProjectionMatrix);
		Vector3 ndcVertexB = Transform(sphere1.center, worldViewProjectionMatrix);
		Vector3 ndcVertexC = Transform(sphere2.center, worldViewProjectionMatrix);
		// スクリーン座標へ変換
		Vector3 screenVerticesA = Transform(ndcVertexA, viewportMatrix);
		Vector3 screenVerticesB = Transform(ndcVertexB, viewportMatrix);
		Vector3 screenVerticesC = Transform(ndcVertexC, viewportMatrix);
		...
		Novice::DrawLine((int)screenVerticesB.x, (int)screenVerticesB.y, (int)screenVerticesC.x, (int)screenVerticesC.y, WHITE);
	

		// グリッド
		DrawGrid(worldViewProjectionMatrix, viewportMatrix);
		// スフィア
		DrawSphere(sphere0, worldViewProjectionMatrix, viewportMatrix,RED);
		DrawSphere(sphere1, worldViewProjectionMatrix, viewportMatrix,GREEN);
		DrawSphere(sphere2, worldViewProjectionMatrix, viewportMatrix,BLUE);
		...
		ImGui::DragFloat3("SphereAscales", &scales[0].x, 0.01f);
		ImGui::DragFloat3("SphereArotates", &rotates[0].x, 0.01f);
		ImGui::DragFloat3("SphereAtranslates", &translates[0].x, 0.01f);
		ImGui::DragFloat3("SphereBscales", &scales[1].x, 0.01f);
		ImGui::DragFloat3("SphereBrotates", &rotates[1].x, 0.01f);
		ImGui::DragFloat3("SphereBtranslates", &translates[1].x, 0.01f);
		ImGui::DragFloat3("SphereCscales", &scales[2].x, 0.01f);
		ImGui::DragFloat3("SphereCrotates", &rotates[2].x, 0.01f);
		ImGui::DragFloat3("SphereCtranslates", &translates[2].x, 0.01f);
		ImGui::End();
